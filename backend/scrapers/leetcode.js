...
/**
 * Fetch the "About Me" field from a LeetCode profile — used for ownership verification.
 * @param {string} username - LeetCode username
 * @returns {string|null}
 */
async function fetchLeetCodeAbout(username) {
  ...
  try {
    ...
    if (response.status === 200 && !response.data.errors) {
      const aboutMe =
        response.data?.data?.matchedUser?.profile?.aboutMe || "";
      logger.info(
        `[VERIFY] LeetCode aboutMe fetched for ${username}: "${aboutMe.substring(0, 50)}..."`
      );
      return aboutMe;
    }
    return null;
    ...
    logger.error(
      `[VERIFY] Error fetching LeetCode aboutMe for ${username}: ${error.message}`
    );
    return null;
  }
}

module.exports = { scrapeLeetCodeProfile, fetchLeetCodeAbout };
